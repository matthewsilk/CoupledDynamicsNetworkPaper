...
#where functions and parameter csvs are located - needs to be set by user
sourcefolder <- "."
paramsfolder <- "params"
...
#where you want to save the results - needs to be set by user 
outputfolder<-file.path("sourcefolder","output","results4")
...
params3<-read.csv(file.path(paramsfolder,"he_params2.csv"))
params3<-params3[,2:4]
...
for(nt in 1:9){
  ...
  for(md in 3:77){
    ...
    for(r in 1:10){
      ...
      l_hea<-params3[(nt-1)*77*10+(md-1)*10+r,3]
      #and an additive effect used to calculate the same parameter for old adults
      ...
      S_E<-0.46/mean(colSums(dis_mat))
      #S_E<-0.2
      
      #lambda for a Poisson draw for the length of this period
      E_I1<-4
      
      #probability of transitioning to serious case for young (daily) ###now no longer used by code
      yI1_I2<-0.01
      #and same for old
      oI1_I2<-0.05
